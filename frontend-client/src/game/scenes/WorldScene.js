...
export class WorldScene extends Phaser.Scene {
  ...
  fetchAndShowDialogue(npcData) {
    ...
    fetch(`http://localhost:3001/api/npcs/${npcData.id}/dialogue`)
      ...
      .then(dialogue => {
        // Launch DialogueUI scene with dialogue data
        this.scene.launch("DialogueUI", {
          ...
          onComplete: (result) => {
            ...
            if (result.trustChange) {
              ...
              }).catch(err => console.error("Failed to record talk:", err))
            }

            // Complete mission objectives related to this NPC
      ...
      .then(missions => {
        ...
        if (available) {
          ...
          .then(() => {
            ...
            const tracker = this.scene.getScene("MissionTracker")
            if (tracker) {
              tracker.setMission(available, [], 0)
            }
          })
