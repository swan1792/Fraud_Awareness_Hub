...
export class PhoneUI extends Phaser.Scene {
  ...
  showContacts(x, y, w, h) {
    ...
    fetch(`${window.location.protocol}//${window.location.hostname}:3001/api/relationships`)
      .then(res => res.json())
      .then(contacts => {
  ...
  doLoad(saveId) {
    ...
    fetch(`${API_BASE}/saves/${saveId}/load`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    })
    .then(res => res.json())
  ...
  doDeleteSave(saveId) {
    fetch(`${API_BASE}/saves/${saveId}`, { method: "DELETE" })
      .then(res => res.json())
      .then(() => {
